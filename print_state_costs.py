...
def main():
    three_quarters_state_avg_rss_dict = json.load(open("three_quarters_state_avg_rss_dict.json"))
    for state in three_quarters_state_avg_rss_dict:
        if (three_quarters_state_avg_rss_dict[state] < 50000):
            print("50000 > Cost, ", state)
        if ((three_quarters_state_avg_rss_dict[state] >= 50000) and (three_quarters_state_avg_rss_dict[state] <= 100000)):
